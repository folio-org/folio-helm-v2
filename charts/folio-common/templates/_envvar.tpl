...
{{- include "folio-common.tplvalues.render" (dict "value" .Values.eureka.envVars "context" $) | nindent 2 }}
{{- end }}
...
{{- range $integration, $config := .Values.integrations -}}
  {{- if eq (include "folio-common.tplvalues.render" (dict "value" $config.enabled "context" $)) "true" }}
    ...
    {{- else if and $.Values.releaseSupport (eq $integration "opensearch") }}
      {{- $secretName = "opensearch-credentials2" -}}  
